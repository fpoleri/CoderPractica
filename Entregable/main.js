...
const sumaCarrito = JSON.parse(localStorage.getItem('sumaTotal')) ?? [];
...
function generarCardCarrito() {
    ...
    <p>
        ...
        -${producto.precio} $
        <button id=${idButton} borrarDelCarrito(${producto.id}) > eliminar </button>     
   </p>
</div>`
        /*productos.forEach((producto) => {
            const idButton = `card-carrito${producto.id}`
...
function agregarCarrito(nuevoProducto) {
    ...
    const elmentoCarrito = JSON.parse(localStorage.getItem('totalCarrito')) ?? [];
    const sumaCarrito = JSON.parse(localStorage.getItem('sumaTotal')) ?? [];
...
function borrarDelCarrito(idProducto) {
    ...
    localStorage.setItem('totalCarrito', carritoJSON)
